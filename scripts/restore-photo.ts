...
if (!title || !filePath) {
  throw new Error('Usage: pnpm tsx scripts/restore-photo.ts "<title>" <path-to-image>');
}
...
if (!blobToken) throw new Error('BLOB_RW_TOKEN is required');
...
if (!contentType) throw new Error(`Unsupported image type: ${ext || '(none)'}`);
...
const backup = JSON.parse(readFileSync('watermarked-photos-backup.json', 'utf8')) as BackupRow[];
const original = backup.find((r) => r.title === title);
if (!original) {
  throw new Error(`No backup entry titled "${title}". Available: ${backup.map((r) => r.title).join(', ')}`);
}

const bytes = readFileSync(filePath);
console.log(`Uploading ${basename(filePath)} (${(bytes.length / 1024 / 1024).toFixed(1)} MB)…`);

const blob = await put(`portfolio/restored/${Date.now()}-${basename(filePath)}`, bytes, {
  access: 'public',
  token: blobToken,
  contentType,
...
try {
  const { rows } = await client.query(
    `INSERT INTO photos (url, title, category_id, sort_order, created_by)
     ...
     RETURNING id`,
    [blob.url, original.title, original.sort_order, original.created_by, original.category_slug],
  );
  if (rows.length === 0) {
    throw new Error(`Category "${original.category_slug}" no longer exists — restore it first.`);
  }
  console.log(`Restored "${original.title}" into ${original.category_slug} at position ${original.sort_order}.`);
} finally {
  await client.end();
}
