...
if (!email) throw new Error('NEW_ADMIN_EMAIL is required');
...
try {
  ...
  const all = await client.query<{ email: string }>('SELECT email FROM users ORDER BY created_at');
  console.log(`Admins on this database: ${all.rows.map((r) => r.email).join(', ')}`);
} finally {
  await client.end();
}
