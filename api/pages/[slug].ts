...
export default async function handler(req: VercelRequest, res: VercelResponse) {
  ...
  const raw = typeof req.query.slug === 'string' ? req.query.slug : req.query.slug?.[0];
  const slug = raw?.trim().toLowerCase();
  if (!slug) return res.status(400).json({ error: 'Missing page address' });
  ...
  try {
    ...
    if (req.method === 'PATCH') {
      ...
      if (slugCheck.slug !== slug) {
        const clash = await sql`SELECT id FROM pages WHERE slug = ${slugCheck.slug} LIMIT 1`;
        if (clash.length > 0) {
      ...
      if (body.content !== undefined && !isEditorDoc(body.content)) {
        return res.status(400).json({ error: 'Page content is not a valid document.' });
      }
    ...
    if (req.method === 'DELETE') {
      if (!user) return res.status(401).json({ error: 'Unauthorized' });
      const deleted = await sql`DELETE FROM pages WHERE slug = ${slug} RETURNING id`;
      if (deleted.length === 0) return res.status(404).json({ error: 'Page not found' });
