...
import { DEFAULT_THEME, isHexColor, findFont } from '../config/theme.js';
...
const readRow = async (siteKey: string): Promise<SiteSettingsRow | null> => {
  const sql = getSql();
  const rows = await sql`SELECT * FROM site_settings WHERE site_key = ${siteKey} LIMIT 1`;
...
export default async function handler(req: VercelRequest, res: VercelResponse) {
  ...
  if (req.method === 'PATCH') {
    ...
    if (typeof body.instagramUrl === 'string' && body.instagramUrl.trim() !== '') {
      instagramUrl = parsePublicHttpUrl(body.instagramUrl);
      if (!instagramUrl) {
        return res.status(400).json({ error: 'Instagram URL must be a full https:// link' });
      }
    }
    ...
    for (const key of ['background', 'foreground', 'accent'] as const) {
      ...
      if (value != null && !isHexColor(value)) {
        return res.status(400).json({ error: `${key} must be a hex color like #1a1a1a` });
      }
    }
    for (const key of ['sansFont', 'serifFont'] as const) {
      const value = themeInput[key];
      if (value != null && !findFont(value as string)) {
        return res.status(400).json({ error: `Unknown font: ${String(value)}` });
      }
    }

    const theme = {
      background: isHexColor(themeInput.background) ? themeInput.background : fallbackTheme.background,
      foreground: isHexColor(themeInput.foreground) ? themeInput.foreground : fallbackTheme.foreground,
      accent: isHexColor(themeInput.accent) ? themeInput.accent : fallbackTheme.accent,
      sansFont: findFont(themeInput.sansFont as string)?.id ?? fallbackTheme.sansFont,
      serifFont: findFont(themeInput.serifFont as string)?.id ?? fallbackTheme.serifFont,
    };
    ...
    try {
      ...
      return res.status(200).json(resolveSiteSettings(site, await readRow(site.key)));
    } catch (e) {
      console.error(e);
