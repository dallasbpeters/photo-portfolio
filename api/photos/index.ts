...
const parsePhotoFields = (body: Record<string, unknown>) => {
  ...
  return {
    ...
    lqip:
      lqipRaw.startsWith("data:image/") && lqipRaw.length <= 4000
        ? lqipRaw
        : null,
    title: typeof body.title === "string" ? sanitizeText(body.title) : "",
    url: parsePublicHttpUrl(typeof body.url === "string" ? body.url : ""),
...
const handlePost = async (
  ...
  const { alt, categoryId, exif, height, lqip, title, url, width } =
    parsePhotoFields(parseJsonBody(req.body));
...
const handleFailure = (e: unknown, res: VercelResponse) => {
  console.error(e);
